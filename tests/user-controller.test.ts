...
test.describe('User management API', () => {
    ...
    test.skip('GET request should return a user by ID', async ({ request }) => {
    ...
    test('DELETE request should return 404 if user is not not found', async ({ request }) => {
        const NewCreated = await request.post(`${baseURL}`);
        expect(NewCreated.status()).toBe(201);
        const responseBody = await NewCreated.json()
        expect(responseBody.id).toBeDefined();

        const newUSerID = responseBody.id;
        const responseDeleted = await request.delete(`${baseURL}` + "/" + `${newUSerID}`);
        expect(responseDeleted.status()).toBe(200);
        const response =  await request.delete(`${baseURL}/${newUSerID}`);
        expect(response.status()).toBe(404);
    });
    })